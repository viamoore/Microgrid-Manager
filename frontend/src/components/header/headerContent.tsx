import { Badge, Button, Popover } from "antd";
import {
  BellFilled,
  ContainerFilled,
  MenuFoldOutlined,
  MenuUnfoldOutlined,
} from "@ant-design/icons";
import { useMicrogrid } from "../../context/useMicrogridContext";
import { useState } from "react";

const content_process = (
  <div className="flex w-full flex-col gap-2">
    <h1 className="mb-4 w-full rounded-sm bg-yellow-400 p-2 text-red-600">
      WORK IN PROGRESS
    </h1>
    <p>No ongoing processes</p>
    <div className="h-0.5 border-t border-gray-200" />
    <Button className="w-full">Clear</Button>
  </div>
);

const content_notification = (
  <div className="flex w-full flex-col gap-2">
    <h1 className="mb-4 w-full rounded-sm bg-yellow-400 p-2 text-red-600">
      WORK IN PROGRESS
    </h1>
    <p>Expect Heavy Rainfall and Low Visability</p>
    <div className="h-0.5 border-t border-gray-200" />
    <Button className="w-full">Clear</Button>
  </div>
);

const HeaderContent = () => {
  const { collapsed, toggleCollapsed } = useMicrogrid();
  const [openNotificaion, setOpenNotificaion] = useState(false);
  const [openProcess, setOpenProcess] = useState(false);

  const handleOpenNotificaion = (newOpen: boolean) => {
    setOpenNotificaion(newOpen);
  };

  const handleOpenProcess = (newOpen: boolean) => {
    setOpenProcess(newOpen);
  };

  return (
    <div className="flex w-full items-center justify-between bg-gray-400">
      <Button
        className="!h-16 !w-16"
        type="text"
        data-testid="collapse-button"
        icon={
          collapsed ? (
            <MenuUnfoldOutlined
              data-testid="Header-Icon-Collapsed"
              className="text-white"
            />
          ) : (
            <MenuFoldOutlined
              data-testid="Header-Icon-Not-Collapsed"
              className="text-white"
            />
          )
        }
        onClick={toggleCollapsed}
      />
      <div
        className={`ml-auto flex items-center ${collapsed ? "mr-[80px]" : "mr-[200px]"}`}
      >
        <Popover
          placement="bottomRight"
          trigger="click"
          open={openProcess}
          onOpenChange={handleOpenProcess}
          content={content_process}
        >
          <Button
            className="!h-16 !w-16 rounded-lg"
            type="text"
            icon={
              <Badge count={0} size="small">
                <ContainerFilled className="text-white" />
              </Badge>
            }
          />
        </Popover>
        <Popover
          placement="bottomRight"
          trigger="click"
          content={content_notification}
          open={openNotificaion}
          onOpenChange={handleOpenNotificaion}
        >
          <Button
            className="!h-16 !w-16 rounded-lg"
            type="text"
            icon={
              <Badge count={1} size="small">
                <BellFilled className="text-white" />
              </Badge>
            }
          />
        </Popover>
      </div>
    </div>
  );
};

export default HeaderContent;
