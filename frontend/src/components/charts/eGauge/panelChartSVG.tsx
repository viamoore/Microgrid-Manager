...
const PanelChartSVG: React.FC<PanelChartSVGProps> = ({
  width,
  height,
  data,
  unit,
  ...
  let div = d3.select(parent.current).select(".tooltip") as d3.Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  ...
  if (div.empty()) {
    div = d3
      ...
      .style("opacity", 0) as d3.Selection<
      HTMLDivElement,
      unknown,
      null,
      undefined
  ...
  const filteredData = useMemo(
    () =>
      data.filter(
        (d) =>
          d.dateTime &&
          d.dateTime.getTime() >= new Date().getTime() - timeRangeLimit,
      ),
    [data, timeRangeLimit],
  );

  const xDomain = useMemo(
    () => d3.extent(filteredData, (d) => d.dateTime) as [Date, Date],
    [filteredData],
  );

  const x = useMemo(
    () => d3.scaleTime().domain(xDomain).range([marginLeft, width]),
    [xDomain, marginLeft, width],
  );
  ...
  useEffect(() => {
    ...
    const svg = d3
      ...
      .attr(
        "style",
        "width: 100%; height: 100%; overflow: visible; font: 10px sans-serif; padding: 4px;",
      );
  ...
  useEffect(() => {
    ...
    const pointermoved = (
      event: React.PointerEvent<SVGSVGElement> | [number, number],
    ) => {
      const [xm, ym] = Array.isArray(event) ? event : d3.pointer(event);
      svg.selectAll(".vertical-line").remove(); // Remove existing line
      if (
        !lastMousePosition ||
        Math.abs(xm - lastMousePosition[0]) > 1 ||
        Math.abs(ym - lastMousePosition[1]) > 1
      ) {
