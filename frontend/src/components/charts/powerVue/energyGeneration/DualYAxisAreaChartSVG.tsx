...
const DualYAxisAreaChartSVG: React.FC<DualYAxisAreaChartSVGProps> = ({
  width,
  height,
  capacity,
  data,
  ...
  const [div, setDiv] = useState<d3.Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  ...
  const otherData: DataTypeWattHour[] = d3.merge(
    Object.values(data).filter((d) => d !== batteryData),
  );
  ...
  const x = useMemo(
    () =>
      d3.scaleTime(d3.extent(batteryData, (d) => d.dateTime) as [Date, Date], [
        marginLeft,
        width - marginRight,
      ]),
    [batteryData, width],
  ...
  useEffect(() => {
    ...
    const svg = d3
      ...
      .attr(
        "style",
        "width: 100%; height: 100%; overflow: hidden; font: 10px sans-serif; padding: 0px;",
      );

    const sources = d3.union(
      otherData.map((d) => d.source),
      ["battery"],
    );

    Array.from(
      otherData.reduce((set, d) => set.add(d.source), new Set<string>()),
    )
      .filter((source) => source !== "battery")
    ...
    svg
      ...
      .call(
        d3
          .axisLeft(y1)
          .ticks(Math.ceil((yMax - yMin) / 2000))
          .tickSize(0),
      ) // Adjust ticks here
    ...
    svg
      ...
      .attr(
        "transform",
        `translate(${width - marginRight}, ${marginBottom - marginTop})`,
      )
      .call(
        d3
          .axisRight(y2)
    ...
    const pointermoved = (event: React.PointerEvent<SVGSVGElement>) => {
      ...
      const closestData =
        xTime.getTime() - d0.dateTime.getTime() >
        d1.dateTime.getTime() - xTime.getTime()
          ? d1
          : d0;
    ...
    const pointerleft = () => {
      ...
      div?.style("left", -1000 + "px").style("top", -1000 + "px");
    };
